	...
	exp.CrawlAfter(func(v *ast.Exp, ctx *ast.VisitContext) error {
		...
		if v.Id != nil {
			...
			for n, t := range closureAt[v] {
				if block := ctx.BlockOf(n); block == nil || (block.Assignments[n] == nil && block.TCFunctions[n] == nil && block.TypeDefs[n] == nil) || (block.Assignments[n] != nil && !block.Assignments[n].Type().IsFunction()) {
